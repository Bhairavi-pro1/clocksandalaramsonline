...
export default function EggTimerClient() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8">
      ...
      <div className="w-auto sm:w-full bg-white dark:bg-[#110624]/60 backdrop-blur-md border border-x-0 sm:border border-slate-200 dark:border-white/5 rounded-none sm:rounded-[2.5rem] p-4 sm:p-8 md:p-12 -mx-4 sm:mx-0 shadow-2xl relative overflow-hidden">
        {/* Decorative glow */}
        ...
        <div className="z-10 w-full flex flex-col md:flex-row justify-between items-center gap-4 md:gap-8">
          <div className="text-center md:text-left space-y-1 sm:space-y-2">
            <h2 className="text-5xl sm:text-7xl md:text-8xl font-black text-slate-800 dark:text-white tracking-tighter tabular-nums drop-shadow-xl leading-none">
              {status === 'running' || status === 'paused' ? formatTime(timeLeft!) : formatTime(calculatedSeconds)}
            </h2>
            <p className="text-xs sm:text-sm md:text-xl text-primary font-bold uppercase tracking-widest">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
        ...
        <div className={`bg-[#110624] p-4 sm:p-8 border border-slate-200 dark:border-white/5 rounded-[1.75rem] sm:rounded-[2.5rem] transition-opacity duration-300 ${status === 'running' ? 'opacity-50 pointer-events-none' : ''}`}>
          <div className="space-y-6 sm:space-y-8 h-full flex flex-col justify-center">
            ...
            <div className="bg-[#110624] p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-primary/20 relative overflow-hidden shadow-inner min-h-[110px] sm:h-[150px] flex flex-col justify-center shrink-0">
               <div className="absolute -top-4 -right-4 w-24 h-24 bg-primary/20 blur-[30px] rounded-full pointer-events-none" />
               <p className="text-[10px] sm:text-xs uppercase tracking-widest text-primary font-black mb-1 sm:mb-2 opacity-80 z-10 relative flex items-center gap-2">
                 Result at {targetMinute} Minutes
               </p>
               <p className="text-white text-sm sm:text-lg md:text-xl font-bold leading-relaxed z-10 relative line-clamp-3">
        ...
        <div className={`bg-[#110624] p-4 sm:p-8 border border-slate-200 dark:border-white/5 rounded-[1.75rem] sm:rounded-[2.5rem] space-y-4 sm:space-y-6 shadow-[0_0_40px_rgba(124,58,237,0.05)] ${status === 'running' ? 'opacity-50 pointer-events-none' : ''}`}>
           ...
           <div className="space-y-4 sm:space-y-6">
              ...
              <div className="space-y-2 sm:space-y-3">
                 <label className="text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] text-slate-400 dark:text-white/40">Egg Size</label>
              ...
              {/* Egg Temp */}
              <div className="space-y-2 sm:space-y-3">
                 <label className="text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] text-slate-400 dark:text-white/40">Starting Temperature</label>
                 ...
                 {eggTemp === 'Room Temperature' && (
                    <div className="mt-2 sm:mt-3 flex items-start gap-2 sm:gap-3 bg-blue-500/10 border border-blue-500/20 p-3 sm:p-4 rounded-xl shadow-inner animate-in zoom-in-95 fade-in duration-300">
                       <Info className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 dark:text-blue-400 flex-shrink-0 mt-0.5" />
                       <p className="text-blue-700 dark:text-blue-200/90 text-xs sm:text-[13px] font-medium leading-relaxed">
              ...
              <div className={`space-y-2 sm:space-y-3 pt-3 sm:pt-4 border-t border-slate-200/50 dark:border-white/5 transition-all ${status === 'running' || status === 'paused' ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
                 ...
                 <div className="flex gap-2 sm:gap-3 items-center">
                    <div className="flex-1 relative group">
                       ...
                       <ChevronDown className="absolute right-3 sm:right-5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 dark:text-white/50 pointer-events-none" />
                    </div>
                    <button 
                       type="button"
