      ...
      <div className="relative w-full max-w-md bg-white dark:bg-[#1a0b36] border border-slate-200 dark:border-primary/30 rounded-[3rem] shadow-[0_0_100px_rgba(124,58,237,0.15)] dark:shadow-[0_0_100px_rgba(124,58,237,0.3)] overflow-hidden animate-in zoom-in-95 duration-300 ring-2 ring-slate-100 dark:ring-primary/20">
        ...
        <div className="p-10 relative z-10 flex flex-col items-center text-center">
          <div className="w-24 h-24 bg-primary/20 rounded-full flex items-center justify-center mb-8 border border-primary/30 animate-bounce shadow-2xl shadow-primary/40">
            {type === 'alarm' ? (
              <Bell size={48} className="text-primary fill-primary" />
            ) : (
              <Hourglass size={48} className="text-primary fill-primary" />
            )}
          </div>

          <div className="space-y-4 mb-10">
            <h2 className="text-4xl font-black text-slate-800 dark:text-white tracking-tight uppercase italic underline decoration-primary decoration-4 underline-offset-8">
              {label}
            </h2>
            {timeText && (
              <p className="text-2xl font-mono font-bold text-primary tracking-widest tabular-nums">
                {timeText}
              </p>
            )}
            <p className="text-slate-400 dark:text-white/40 text-sm font-bold uppercase tracking-[0.3em]">
              Action Required
            </p>
          </div>
          ...
          <button 
            onClick={onClose}
            className="w-full bg-primary hover:bg-primary/90 text-white font-black py-5 rounded-2xl shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-3 active:scale-95 group cursor-pointer"
          >
            <X size={24} className="group-hover:rotate-90 transition-transform duration-300 text-white" />
            DISMISS ALERT
          </button>
        </div>
      </div>
