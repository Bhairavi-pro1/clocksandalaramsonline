...
export default function SmallClockCard({ country, city, timezone, onRemove }: SmallClockCardProps) {
  ...
  return (
    <div className="relative p-3.5 sm:p-7 rounded-2xl sm:rounded-[1.8rem] bg-slate-100/50 dark:bg-[#1a0b2e]/40 border border-slate-200/60 dark:border-violet-500/20 hover:border-slate-300 dark:hover:border-violet-500/40 transition-all duration-500 hover:shadow-[0_0_30px_rgba(124,58,237,0.1)] group">
      {/* Top Header Row */}
      ...
      <div className="flex flex-col items-center justify-center space-y-1 sm:space-y-2 relative pointer-events-none w-full">
        ...
        <div className="text-xl sm:text-[2.65rem] font-bold tracking-tight text-slate-900 dark:text-white leading-none py-1 sm:py-2 font-tabular drop-shadow-[0_0_20px_rgba(124,58,237,0.3)]">
          {time ? time.toFormat(mounted && is24Hour ? 'HH:mm:ss' : 'hh:mm:ss') : '00:00:00'}
          {time && (!mounted || !is24Hour) && (
            <span className="text-[9px] sm:text-sm font-normal text-slate-500 dark:text-white/40 ml-0.5 sm:ml-1.5 uppercase tracking-tighter">{time.toFormat('a')}</span>
          )}
        </div>

        <div className="flex flex-col items-center space-y-0.5 sm:space-y-1 w-full">
          <p className="hidden sm:block text-[11px] font-semibold text-slate-600 dark:text-white/70 w-full text-center truncate">
            {time ? time.toFormat('cccc, LLLL d, yyyy') : 'Loading...'}
          </p>
          <p className="block sm:hidden text-[9px] font-semibold text-slate-600 dark:text-white/70 w-full text-center truncate">
